...
DECLARE
    ...
    v_sql       text    := 'SELECT * FROM public.file WHERE TRUE';
BEGIN
...
-- +migrate StatementBegin
CREATE OR REPLACE FUNCTION public.file_updated_since(_since timestamp without time zone) 
RETURNS SETOF public.file
LANGUAGE sql
STABLE
AS $fn$
