...
func TestFindEntitiesByContentForProduct(t *testing.T) {
	...
	_, err = db.FindEntitiesByContent(ctx, oam.Product, after, 1, dbt.ContentFilters{
		"unique_id": uniqueID,
	})
	assert.Error(t, err, "Expected error when finding entity with CreatedAt after its creation time")

	ents, err := db.FindEntitiesByContent(ctx, oam.Product, before, 1, dbt.ContentFilters{
		"unique_id": uniqueID,
	})
	...
	for k, v := range map[string]string{
		...
		ents, err := db.FindEntitiesByContent(ctx, oam.Product, before, 0, dbt.ContentFilters{k: v})
...
func TestFindEntitiesByTypeForProduct(t *testing.T) {
	db, err := New(SQLiteMemory, "")
	assert.NoError(t, err, "Failed to create the in-memory sqlite database")
	assert.NotNil(t, db, "Asset database should not be nil")
	defer func() { _ = db.Close() }()
	...
	atype := oam.Product
	atypestr := "Product"
	ent, err := db.CreateAsset(ctx, &oamplat.Product{
		ID:   key1,
		Name: "fake name 1",
		Type: "fake type 1",
	})
