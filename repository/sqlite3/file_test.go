...
func TestFindEntitiesByTypeForFile(t *testing.T) {
	db, err := New(SQLiteMemory, "")
	assert.NoError(t, err, "Failed to create the in-memory sqlite database")
	assert.NotNil(t, db, "Asset database should not be nil")
	defer func() { _ = db.Close() }()
	...
	ent, err := db.CreateAsset(ctx, &oamfile.File{URL: key1})
	...
	ent, err = db.CreateAsset(ctx, &oamfile.File{URL: key2})
	...
	ent, err = db.CreateAsset(ctx, &oamfile.File{URL: key3})
