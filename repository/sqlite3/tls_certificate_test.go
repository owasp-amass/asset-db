...
func TestCreateAssetForTLSCertificate(t *testing.T) {
	...
	notbefore := time.Now().Add(-1 * time.Hour).UTC().Format("2006-01-02T15:04:05Z07:00")
	notafter := time.Now().Add(365 * 24 * time.Hour).UTC().Format("2006-01-02T15:04:05Z07:00")
...
func TestFindEntitiesByTypeForTLSCertificate(t *testing.T) {
	db, err := New(SQLiteMemory, "")
	assert.NoError(t, err, "Failed to create the in-memory sqlite database")
	assert.NotNil(t, db, "Asset database should not be nil")
	defer func() { _ = db.Close() }()
	...
	since1 := time.Now()
	notbefore := since1.UTC().Format("2006-01-02T15:04:05Z07:00")
	notafter := since1.Add(24 * time.Hour).UTC().Format("2006-01-02T15:04:05Z07:00")
	time.Sleep(100 * time.Millisecond)

	key1 := "Fake1"
	atype := oam.TLSCertificate
	atypestr := "TLSCertificate"
	ent, err := db.CreateAsset(ctx, &oamcert.TLSCertificate{
		Version:           "3",
		SerialNumber:      key1,
