...
func (suite *PostgresContactRecordTestSuite) TestFindEntitiesByContentForContactRecord() {
	...
	_, err = suite.db.FindEntitiesByContent(ctx, oam.ContactRecord, after, 1, dbt.ContentFilters{
		"discovered_at": discovered,
	})
	assert.Error(t, err, "Expected error when finding entity with CreatedAt after its creation time")

	ents, err := suite.db.FindEntitiesByContent(ctx, oam.ContactRecord, before, 1, dbt.ContentFilters{
		"discovered_at": discovered,
	})
	...
	ents, err = suite.db.FindEntitiesByContent(ctx, oam.ContactRecord, before, 0, dbt.ContentFilters{
		"discovered_at": discovered,
	})
...
func (suite *PostgresContactRecordTestSuite) TestFindEntitiesByTypeForContactRecord() {
	t := suite.T()
	...
	ent, err := suite.db.CreateAsset(ctx, &oamcon.ContactRecord{DiscoveredAt: key1})
	...
	ent, err = suite.db.CreateAsset(ctx, &oamcon.ContactRecord{DiscoveredAt: key2})
	...
	ent, err = suite.db.CreateAsset(ctx, &oamcon.ContactRecord{DiscoveredAt: key3})
