...
func (suite *PostgresRepoTestSuite) TestCreateAssetForService() {
	t := suite.T()
...
func (suite *PostgresRepoTestSuite) TestFindEntitiesByContentForService() {
	t := suite.T()
	...
	_, err = suite.db.FindOneEntityByContent(ctx, oam.Service, after, dbt.ContentFilters{
		"unique_id": uniqueID,
	})
	assert.Error(t, err, "Expected error when finding entity with CreatedAt after its creation time")

	found, err := suite.db.FindOneEntityByContent(ctx, oam.Service, before, dbt.ContentFilters{
		"unique_id": uniqueID,
	})
	...
	for k, v := range map[string]string{
		...
		ents, err := suite.db.FindEntitiesByContent(ctx, oam.Service, before, dbt.ContentFilters{k: v})
