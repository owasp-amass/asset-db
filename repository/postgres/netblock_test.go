...
type PostgresNetblockTestSuite struct {
	suite.Suite
	container *testhelpers.PostgresContainer
	db        *PostgresRepository
}

func TestPostgresNetblockTestSuite(t *testing.T) {
	suite.Run(t, new(PostgresNetblockTestSuite))
}

func (suite *PostgresNetblockTestSuite) SetupSuite() {
	var err error
...
func (suite *PostgresNetblockTestSuite) TearDownSuite() {
	if err := suite.container.Terminate(context.Background()); err != nil {
		log.Fatalf("error terminating postgres container: %s", err)
	}
}

func (suite *PostgresNetblockTestSuite) TestCreateAssetForNetblock() {
	t := suite.T()
...
func (suite *PostgresNetblockTestSuite) TestFindEntitiesByContentForNetblock() {
	t := suite.T()
	...
	cidrstr := cidr.String()
	_, err = suite.db.FindEntitiesByContent(ctx, oam.Netblock, after, 1, dbt.ContentFilters{
		"cidr": cidrstr,
	})
	assert.Error(t, err, "Expected error when finding entity with CreatedAt after its creation time")

	ents, err := suite.db.FindEntitiesByContent(ctx, oam.Netblock, before, 1, dbt.ContentFilters{
		"cidr": cidrstr,
	})
	...
	ents, err = suite.db.FindEntitiesByContent(ctx, oam.Netblock, before, 0, dbt.ContentFilters{
		"cidr": cidrstr,
	})
...
func (suite *PostgresNetblockTestSuite) TestFindEntitiesByTypeForNetblock() {
	t := suite.T()
	...
	cidr := netip.MustParsePrefix("72.237.4.0/24")
	key1 := cidr.String()
	ent, err := suite.db.CreateAsset(ctx, &oamnet.Netblock{
		CIDR: cidr,
		Type: iptype,
	})
