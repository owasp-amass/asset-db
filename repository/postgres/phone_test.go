...
func (suite *PostgresPhoneTestSuite) TestFindEntitiesByContentForPhone() {
	...
	_, err = suite.db.FindEntitiesByContent(ctx, oam.Phone, after, 1, dbt.ContentFilters{
		"e164": e164,
	})
	assert.Error(t, err, "Expected error when finding entity with CreatedAt after its creation time")

	ents, err := suite.db.FindEntitiesByContent(ctx, oam.Phone, before, 1, dbt.ContentFilters{
		"e164": e164,
	})
	...
	ents, err = suite.db.FindEntitiesByContent(ctx, oam.Phone, before, 0, dbt.ContentFilters{
		"e164": e164,
	})
...
func (suite *PostgresPhoneTestSuite) TestFindEntitiesByTypeForPhone() {
	t := suite.T()
	...
	atype := oam.Phone
	atypestr := "Phone"
	ent, err := suite.db.CreateAsset(ctx, &oamcon.Phone{
		E164: key1,
		Raw:  "fake number 1",
	})
