...
func (suite *PostgresPersonTestSuite) TestCreateAssetForPerson() {
	...
	found, err := suite.db.FindEntityById(ctx, person.ID)
	...
	err = suite.db.DeleteEntity(ctx, person.ID)
	assert.NoError(t, err, "Failed to delete entity by ID for the Person")

	_, err = suite.db.FindEntityById(ctx, person.ID)
...
func (suite *PostgresPersonTestSuite) TestFindEntitiesByContentForPerson() {
	...
	_, err = suite.db.FindEntitiesByContent(ctx, oam.Person, after, 1, dbt.ContentFilters{
		"unique_id": uniqueID,
	})
	assert.Error(t, err, "Expected error when finding entity with CreatedAt after its creation time")

	ents, err := suite.db.FindEntitiesByContent(ctx, oam.Person, before, 1, dbt.ContentFilters{
		"unique_id": uniqueID,
	})
	...
	for k, v := range map[string]string{
		...
		ents, err := suite.db.FindEntitiesByContent(ctx, oam.Person, before, 0, dbt.ContentFilters{k: v})
...
func (suite *PostgresPersonTestSuite) TestFindEntitiesByTypeForPerson() {
	t := suite.T()
	...
	atype := oam.Person
	atypestr := "Person"
	ent, err := suite.db.CreateAsset(ctx, &people.Person{
		ID:       key1,
