...
func (suite *PostgresRepoTestSuite) TestCreateEdgeProperty() {
	...
	tag, err := suite.db.CreateEdgeProperty(ctx, edge, prop)
	...
	err = suite.db.DeleteEdge(ctx, edge.ID)
...
func (suite *PostgresRepoTestSuite) TestFindEdgeTags() {
	...
	tag1, err := suite.db.CreateEdgeProperty(ctx, edge, prop1)
	assert.NoError(t, err, "Failed to create tag for the edge")
	...
	before2 := time.Now()
	time.Sleep(100 * time.Millisecond)
	tag2, err := suite.db.CreateEdgeProperty(ctx, edge, prop2)
	assert.NoError(t, err, "Failed to create tag for the edge")
	...
	for tname, test := range tests {
		tags, err := suite.db.FindEdgeTags(ctx, test.edge, test.since, test.names...)
		if test.count == 0 {
	...
	for name, tag := range map[string]*dbt.EdgeTag{
		...
		err = suite.db.DeleteEdgeTag(ctx, tag.ID)
