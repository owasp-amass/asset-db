...
func (suite *PostgresIdentifierTestSuite) TestFindEntitiesByContentForIdentifier() {
	...
	_, err = suite.db.FindEntitiesByContent(ctx, oam.Identifier, after, 1, dbt.ContentFilters{
		"unique_id": unique_id,
	})
	assert.Error(t, err, "Expected error when finding entity with CreatedAt after its creation time")

	ents, err := suite.db.FindEntitiesByContent(ctx, oam.Identifier, before, 1, dbt.ContentFilters{
		"unique_id": unique_id,
	})
	...
	ents, err = suite.db.FindEntitiesByContent(ctx, oam.Identifier, before, 0, dbt.ContentFilters{
		"unique_id": unique_id,
	})
...
func (suite *PostgresIdentifierTestSuite) TestFindEntitiesByTypeForIdentifier() {
	t := suite.T()
	...
	ent, err := suite.db.CreateAsset(ctx, &oamgen.Identifier{
		UniqueID: key1,
		ID:       "fake1@gmail.com",
