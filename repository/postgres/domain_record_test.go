...
func (suite *PostgresDomainRecordTestSuite) TestFindEntitiesByContentForDomainRecord() {
	...
	_, err = suite.db.FindEntitiesByContent(ctx, oam.DomainRecord, after, 1, dbt.ContentFilters{
		"domain": domain,
	})
	assert.Error(t, err, "Expected error when finding entity with CreatedAt after its creation time")

	ents, err := suite.db.FindEntitiesByContent(ctx, oam.DomainRecord, before, 1, dbt.ContentFilters{
		"domain": domain,
	})
	...
	ents, err = suite.db.FindEntitiesByContent(ctx, oam.DomainRecord, before, 0, dbt.ContentFilters{
		"name": recordName,
	})
	...
	ents, err = suite.db.FindEntitiesByContent(ctx, oam.DomainRecord, time.Time{}, 0, dbt.ContentFilters{
		"id": objectID,
	})
	assert.NoError(t, err, "Failed to find entities by content for the DomainRecord")
	assert.Len(t, ents, 1, "Expected to find exactly one entity by content for the DomainRecord")
...
func (suite *PostgresDomainRecordTestSuite) TestFindEntitiesByTypeForDomainRecord() {
	t := suite.T()
	...
	ent, err := suite.db.CreateAsset(ctx, &oamreg.DomainRecord{
		Domain:         key1,
