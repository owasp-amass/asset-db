...
type PostgresFQDNTestSuite struct {
	suite.Suite
	container *testhelpers.PostgresContainer
	db        *PostgresRepository
}

func TestPostgresFQDNTestSuite(t *testing.T) {
	suite.Run(t, new(PostgresFQDNTestSuite))
}

func (suite *PostgresFQDNTestSuite) SetupSuite() {
	var err error
...
func (suite *PostgresFQDNTestSuite) TearDownSuite() {
	if err := suite.container.Terminate(context.Background()); err != nil {
		log.Fatalf("error terminating postgres container: %s", err)
	}
}

func (suite *PostgresFQDNTestSuite) TestCreateAssetForFQDN() {
	t := suite.T()
...
func (suite *PostgresFQDNTestSuite) TestFindEntitiesByContentForFQDN() {
	t := suite.T()
	...
	_, err = suite.db.FindEntitiesByContent(ctx, oam.FQDN, after, 1, dbt.ContentFilters{
		"name": name,
	})
	assert.Error(t, err, "Expected error when finding entity with CreatedAt after its creation time")

	ents, err := suite.db.FindEntitiesByContent(ctx, oam.FQDN, before, 1, dbt.ContentFilters{
		"name": name,
	})
	...
	ents, err = suite.db.FindEntitiesByContent(ctx, oam.FQDN, before, 0, dbt.ContentFilters{
		"name": name,
	})
...
func (suite *PostgresFQDNTestSuite) TestFindEntitiesByTypeForFQDN() {
	t := suite.T()
	...
	ent, err := suite.db.CreateAsset(ctx, &oamdns.FQDN{
		Name: key1,
	})
